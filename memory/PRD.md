...
## 2026-09-13 (sesi 8) — FASE SL: Penerimaan berbasis SCAN LABEL SUPPLIER
**Permintaan asli:** lanjutkan repo `pandeyoga/KNHOST`; implementasi rencana "Scan – Konfirmasi – Tempel – Simpan bin"
(petugas tidak mengetik ulang label roll supplier; identitas dari scan + master, manusia hanya konfirmasi ukuran fisik).
**Keputusan pemilik:** (a) semua format label (GS1 / QR JSON / teks berpemisah / regex, dipilih per supplier, default auto);
(b) toleransi selisih label vs aktual **2%** → flag QC; (c) input = scanner HID **dan** kamera.

**Arsitektur:**
- `services/label_decoder_service.py` — decoder murni (GS1 AI 01·10·21·240·241·30·310n·311n·323n·91; QR JSON alias; delimited; regex).
- `schemas_scan_label.py`; `SupplierCreate.label_pattern`; PATCH supplier whitelist `label_pattern`.
- `routers/inbound_scan_label.py`: `POST /api/inbound/tasks/{id}/scan-label` (kode galat `UNDECODABLE|UNMAPPED|NOT_IN_TASK|NO_ROLL_NO|NO_QTY|OVER_PO|DUPLICATE`, idempoten `supplier_id+supplier_roll_no`),
  `GET …/scanned-rolls`, `POST /api/inbound/rolls/{id}/confirm-measure`, `POST …/putaway` (bin dari master), `DELETE …/scan`,
  `GET /api/inbound/scan-label/stats`, `GET/PUT /api/suppliers/{id}/label-pattern`, `POST /api/suppliers/label-pattern/test`.
- `inbound_receiving.complete`: roll `receiving` hasil scan = SSOT → difinalkan (tidak dibuat ulang); `receipt_variance` → task + `purchase_orders.receipt_variances`.
- Konfigurasi `receiving.label_variance_tolerance_percent` (default 2) di `config_catalog_ops.py`.
- FE: `features/wms/inbound/ScanLabelPanel.jsx` · `ScannedRollRow.jsx` · `UnreadableLabelModal.jsx`; toggle "Scan Label Supplier / Input Manual" di `InboundTaskPanel.jsx`;
  `InboundScanInterface.jsx` selesaikan GR langsung bila ada roll hasil scan; `features/purchasing/SupplierLabelPatternEditor.jsx` di form Supplier; `RollsTable.jsx` alias supplier; status roll `receiving`.
- Lingkungan: FE bundel statis → `bash scripts/rebuild_frontend.sh` setelah edit FE; backend `.env` `CORS_ORIGINS` eksplisit.

**Backlog FASE SL:** ~~P1 tampilkan `receipt_variances` di detail/papan PO & finance~~ · ~~P1 encode EPC RFID + cetak label KN dari panel scan~~ ·
~~P2 alias supplier di katalog MD~~ · ~~P2 halaman laporan % label tidak terbaca per supplier~~ · P2 pola label per Supplier Item.

## 2026-09-16 (sesi 9a — diuji iteration_16, belum dibukukan sesi lalu) — RFID Auto-Tag + Alias Katalog MD
- Setelan `receiving.auto_rfid_on_scan` (default true): tiap roll hasil scan label otomatis dapat EPC (`rfid_epc`, `tracking_mode=rfid`, `rfid_tag_id` `rtag_*`); `POST /api/inbound/rolls/{id}/tag` (manual, 400 bila sudah bertag); tombol cetak label KN per baris + blok `scan-label-print-actions` (popup / antrean printer) di `ScanLabelPanel`.
- Alias supplier di katalog MD: `product-templates/{id}` → `variants[].supplier_codes`; `inventory/balances` → `supplier_codes`; FE `catalog-supplier-alias-*`, `catalog-selected-supplier-alias`, `balance-supplier-alias-*`.
- Bukti: testing agent iteration_16 backend 9/9 · frontend 100%; pytest `backend/tests/test_fase_sl_rfid_alias.py`.

## 2026-09-16 (sesi 9b) — Repo `njifssgb/KN` diimpor · P1 selisih penerimaan di PO & finance · P2 laporan label
- Lingkungan: clone → rsync tanpa `.env` → `CORS_ORIGINS` eksplisit → `.restore_env.sh` (seed OK, fondasi LENGKAP, build OK). `pip install -r requirements.txt` tetap gagal karena `emergentintegrations`/`litellm` (sudah ada di base image) → pasang tanpa dua baris itu.
- **P1 `receipt_variances`** (ditulis `inbound_receiving.complete` saat GR selesai) kini DIBACA: `po_board_service.receipt_variance_summary()` → kolom `receipt_variance` baris papan (`{count, diff_qty, flagged_rolls, label, notes}` / `None`); `vendor_bill_service.build_billing_context()` → `receipt_variances` untuk form Vendor Bill. FE: `features/admin/po/POReceiptVariancePanel.jsx` (+`receiptVarianceSummary`) dipakai `PODetailPanel` (`po-receipt-variances`), `POCompactPanel` (`po-compact-receipt-variances`), badge baris `po-row-variance-{id}`, papan `po-board-variance-{po_id}`, form tagihan `vb-receipt-variances`.
- **P2 laporan kualitas label** `features/wms/inbound/ScanLabelStatsCard.jsx` (collapsible, data `GET /api/inbound/scan-label/stats`) di bawah grid tugas Barang Masuk.
- `ux_audit --strict` memerah pada 2 `<select>` bawaan warisan FASE SL (`ScannedRollRow`, `UnreadableLabelModal`) → diganti `KNSelect`; guard hijau (ux_audit, verify_po_board, verify_api_contract, error_notice, picker_portal, modal_dismiss).
- Data uji: task `wms_7d68b73ebcc1` (PO-00014) diselesaikan dengan 3 roll scan (1 roll RL-00058 label 120 vs aktual 110 → flag QC) → PO `po_78d343ce4efd` completed + 1 `receipt_variance`.
- Bukti: pytest `backend/tests/test_fase_sl_receipt_variance.py` 6 passed; testing agent iteration_17 frontend 100% (5 skenario, nol error konsol).
- Catatan: peran `finance` demo tidak memegang `vendor_bill.view` (matriks izin lama) — tagihan supplier dikerjakan admin/manager.

**Backlog berikutnya:** ~~P2 pola label per Supplier Item~~ · (opsional) beri `vendor_bill.view` ke peran finance bila pemilik mau · sisa roadmap EPIC 7 multi-currency (ditunda tanpa bukti kebutuhan).

## 2026-09-16 (sesi 9c) — P2 Pola Label per Barang Supplier (supplier campur-label)
- `supplier_items.label_pattern` (opsional; `{}` di PATCH = hapus → ikut supplier) lewat `SupplierItemCreate/Patch`; service `_clean_label_pattern()` + `label_pattern_candidates(supplier×produk)`.
- Decoder `ldec.decode_first(raw, [(source, pattern)…])`: pola barang dicoba dulu, lalu pola supplier; hasil membawa `pattern_source`. Label tanpa kode barang yang cocok pola barang → barang tersirat dari polanya (`supplier_sku` diisi).
- `scanned-rolls` → `item_label_patterns[]`; FE badge `scan-label-item-patterns` di panel scan; form Barang Supplier: checkbox `supplier-item-own-pattern` + `SupplierLabelPatternEditor` (uji scan contoh); badge baris `supplier-item-label-pattern-{id}`.
- Data demo: `sit_b5ed6c7e1bc9` (NTT-IKAT-GRD) berpola GS1; supplier Ntt Weaving tetap `auto`.
- Bukti: pytest `backend/tests/test_fase_sl_item_label_pattern.py` 7 passed; testing agent iteration_18 frontend 100% (roll uji di-undo, task `wms_f808e668a2e7` bersih); guard ux_audit/api_contract/modal_dismiss hijau.
